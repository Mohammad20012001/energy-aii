...
class EmbeddedMapSystem {
    ...
    initEmbeddedMap() {
        try {
            ...
            const mapElement = document.getElementById('embedded-map');
            if (!mapElement) {
            ...
            console.log('High-performance embedded map initialized (original settings restored)');
        } catch (error) {
            console.error('Error initializing embedded map:', error);
            // إضافة رسالة خطأ للمستخدم
            const mapElement = document.getElementById('embedded-map');
            if (mapElement) {
                mapElement.innerHTML = '<div style="display: flex; align-items: center; justify-content: center; height: 100%; color: #ff7200; font-weight: bold;">Map loading failed. Please refresh the page.</div>';
            }
        }
    }
    ...
    initFullMap() {
        try {
            ...
            if (!fullMapElement) {
                console.error('Full map container element not found');
            ...
            this.fullMap = L.map('full-map', {
                preferCanvas: true,
                renderer: L.canvas(),
                zoomControl: true, // تفعيل أدوات التكبير الافتراضية
                maxZoom: 25, // تحديد أقصى تكبير
                minZoom: 1 // تحديد أدنى تكبير
            }).setView([31.9539, 35.9106], 25);
            ...
            this.hideLoadingIndicator();
            console.log('Professional full map initialized with enhanced zoom capabilities');
        } catch (error) {
            ...
            const fullMapElement = document.getElementById('full-map');
            if (fullMapElement) {
                fullMapElement.innerHTML = '<div style="display: flex; align-items: center; justify-content: center; height: 100%; color: #ff7200; font-weight: bold; font-size: 18px;">Full map loading failed. Please try again.</div>';
            }
        }
    }

    createMapLayers() {
        // إعدادات مشتركة للطبقات
        const commonOptions = {
            maxZoom: 25,
            minZoom: 1,
        ...
        // إضافة معالجات الأخطاء لكل طبقة
    ...
    addAdvancedControls() {
        ...
        // إزالة لوحة التحكم القديمة - سنستخدم المصغرة فقط
                ...
                <div class="panel-content" id="panel-content">
                    ...
                    <div class="compact-section">
                        ...
                        <div class="compact-tools">
                            <button class="tool-icon" id="zoom-in" title="Zoom In">➕</button>
                            <button class="tool-icon" id="zoom-out" title="Zoom Out">➖</button>
    ...
    setupAdvancedControlEvents() {
        ...
        // تبديل طبقات الطاقة مع التحقق من وجود العناصر
        const solarToggle = document.getElementById('toggle-solar');
        if (solarToggle) {
        ...
        const nuclearToggle = document.getElementById('toggle-nuclear');
        if (nuclearToggle) {
            nuclearToggle.addEventListener('change', (e) => {
        ...
        // أدوات إضافية مع التحقق من وجود العناصر
        ...
        if (resetViewBtn) {
            resetViewBtn.addEventListener('click', () => {
                if (this.fullMap) {
                    this.fullMap.setView([31.9539, 35.9106], 25);
                }
            });
        }
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        if (typeof L !== 'undefined') {
            console.log('Initializing Enhanced Map System with Zoom Level 25...');
            ...
            console.log('Enhanced Map System initialized successfully');
        } else {
            console.error('Leaflet library not loaded - Map functionality will not work');
            // إضافة رسالة خطأ للمستخدم
            const mapElement = document.getElementById('embedded-map');
            if (mapElement) {
                mapElement.innerHTML = '<div style="display: flex; align-items: center; justify-content: center; height: 100%; color: #ff7200; font-weight: bold;">Map library not loaded. Please refresh the page.</div>';
            }
        }
    } catch (error) {
        console.error('Critical error initializing map system:', error);
        // إضافة رسالة خطأ للمستخدم
        const mapElement = document.getElementById('embedded-map');
        if (mapElement) {
            mapElement.innerHTML = '<div style="display: flex; align-items: center; justify-content: center; height: 100%; color: #ff7200; font-weight: bold;">Map system failed to initialize. Please refresh the page.</div>';
        }
    }
});
